...
def _record_crowd_data(
    ...
    data = [
        ...
        human_count,
        violate_count,
        int(restricted_entry),
        int(abnormal_activity),
    ]
...
def create_red_gradient(width, height, horizontal=True):
    ...
    if horizontal:
        ...
        gradient[:, :, 2] = np.tile(
            np.linspace(0, 255, height, dtype=np.uint8), (width, 1)
        )
...
def apply_red_gradient_border(frame, border_size=20):
    ...
    bottom_gradient = cv2.flip(
        top_gradient, 0
    )  # Flip top gradient to get bottom gradient
    left_gradient = create_red_gradient(border_size, height, horizontal=False)
    right_gradient = cv2.flip(
        left_gradient, 1
    )  # Flip left gradient to get right gradient

    # Apply red gradients to the borders
    frame[:border_size, :] = cv2.addWeighted(
        frame[:border_size, :], 0.5, top_gradient, 0.5, 0
    )
    frame[-border_size:, :] = cv2.addWeighted(
        frame[-border_size:, :], 0.5, bottom_gradient, 0.5, 0
    )
    frame[:, :border_size] = cv2.addWeighted(
        frame[:, :border_size], 0.5, left_gradient, 0.5, 0
    )
    frame[:, -border_size:] = cv2.addWeighted(
        frame[:, -border_size:], 0.5, right_gradient, 0.5, 0
    )
...
async def video_process(
    cap, frame_size, net, ln, encoder, tracker, movement_data_writer, crowd_data_writer
):
    def _calculate_FPS():
        t1 = time.time() - t0
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            if RE_CHECK:
                RE = False
                if (current_datetime.time() > RE_START_TIME) and (
                    current_datetime.time() < RE_END_TIME
                ):
            ...
            if (
                ...
                for i, track in enumerate(humans_detected):
                    ...
                    if SD_CHECK:
                        if len(humans_detected) >= 2:
                            ...
                            for j, track_2 in enumerate(
                                humans_detected[i + 1 :], start=i + 1
                            ):
                                if HIGH_CAM:
                    ...
                    if RE:
                        cv2.rectangle(
                            frame, (x + 5, y + 5), (w - 5, h - 5), RGB_COLORS["red"], 5
                        )
                ...
                if len(humans_detected) > ABNORMAL_MIN_PEOPLE:
                    if (
                        len(abnormal_individual) / len(humans_detected)
                        > ABNORMAL_THRESH
                    ):
                        ABNORMAL = True
            ...
            loop.create_task(
                await send_frame(
                    session, "https://backend-aeh7hwqzuq-oe.a.run.app/video_feed", frame
                )
            )
